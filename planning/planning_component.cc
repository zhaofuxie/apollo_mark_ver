...
bool PlanningComponent::Init() {
  ...
  if (FLAGS_use_navigation_mode) {
    ...
    ADEBUG <<"Create NaviPlanning " <<"FLAGS_use_navigation_mode ="<<FLAGS_use_navigation_mode;
  } else {
...
bool PlanningComponent::Proc(
  ...
  {
    std::lock_guard<std::mutex> lock(mutex_);
    local_view_.chassis = std::make_shared<Chassis>(chassis_);
  }
  {
    std::lock_guard<std::mutex> lock(mutex_);
    local_view_.localization_estimate = std::make_shared<LocalizationEstimate>(LocalizationEstimate);
  }
