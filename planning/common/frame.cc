...
//更新参考线的优先级，其中key为lane的id,value为优先级。当前的lane是hdmap::RouteSegments数据类型
...
//根据reference_lines创建reference_line_info_(list数据类型)，并且添加障碍物到数组
bool Frame::CreateReferenceLineInfo(
  ...
  //1.添加reference_line_info_添加成员，这里分别
...
//返回第一个变道类型的参考线，如果没有找到，则返回reference_line_info_最后一个参考线
const ReferenceLineInfo *Frame::FindTargetReferenceLineInfo() {
  ...
  //返回 list数据类型的reference_line_info_最后一条参考线

  return target_reference_line_info;
}


//在数据类型为listreference_line_info_ 中找到变道类型并且不能行驶的参考线，没有找到则返回空
