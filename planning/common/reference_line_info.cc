...
// //Chef modify signal with curve
// double point.path_point().kappa()

// void ReferenceLineInfo::SetTurnSignalBasedOnLaneCurve(double C_kappa,
// common::VehicleSignal* vehicle_signal)
//   {
//   if (C_kappa < 0.17 && C_kappa  > 0.0)
//     {
//     vehicle_signal->set_turn_signal(VehicleSignal::TURN_LEFT)
//     }
//     else if (C_kappa > -0.17 && C_kappa <0.0)
//     {
...
void ReferenceLineInfo::SetTurnSignalBasedOnLaneTurnType(
  ...
  // chef modify Set turn signal based on kappa
  const auto& path_point = adc_planning_point_.path_point();
  const auto& point_kappa_ = path_point.kappa();
  const double kRightKappaLimit = -0.002;
  const double kLeftKappaLimit = 0.002;
  // AINFO << "point_kappa_ = " << point_kappa_;
  if (point_kappa_ < kRightKappaLimit) {
    vehicle_signal->set_turn_signal(VehicleSignal::TURN_RIGHT);
  } else if (point_kappa_ > kLeftKappaLimit) {
    vehicle_signal->set_turn_signal(VehicleSignal::TURN_LEFT);
  } else {
    vehicle_signal->set_turn_signal(VehicleSignal::TURN_NONE);
  }

  // if (point_kappa_ < kpluskappalimit &&  point_kappa_ > 0.0) {
  //   vehicle_signal->set_turn_signal(VehicleSignal::TURN_LEFT);
  // } else if (point_kappa_ > kminuskappalimit &&  point_kappa_ < 0.0) {
  //   vehicle_signal->set_turn_signal(VehicleSignal::TURN_RIGHT);
  // } else {
  //   vehicle_signal->set_turn_signal(VehicleSignal::TURN_NONE);
  // }
