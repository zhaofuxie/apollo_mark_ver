...
ReferenceLineProvider::ReferenceLineProvider(
  ...
  //Apollo 参考线平滑类别选择，当前默认平滑类别为：discrete_points_smoother_config
  //DiscretePointsReferenceLineSmoother 默认方法为FEM_POS_DEVIATION_SMOOTHING
...
bool ReferenceLineProvider::UpdateRoutingResponse(
  ...
  AINFO<<"UpdateRouting "<<has_routing_;//debug purpose
...
bool ReferenceLineProvider::CreateReferenceLine(
  ...
  if (is_new_routing || !FLAGS_enable_reference_line_stitching) {
    //A.参考线平滑，条件enable_reference_line_stitching设置为false,即不允许参考线拼接操作
    ...
    return true;
  } else {  // stitching reference line 允许参考线拼接操作
    for (auto iter = segments->begin(); iter != segments->end();) {
      reference_lines->emplace_back();
...
bool ReferenceLineProvider::ExtendReferenceLine(const VehicleState &state,
  ...
  //若在同一平滑参考线上，计算车辆当前位置与历史平滑参考线终点的距离，距离超过阈值，则可以复用这条历史参考线，
  //否则长度不够需要拼接           
  common::SLPoint sl_point;
  Vec2d vec2d(state.x(), state.y());
  LaneWaypoint waypoint;
  if (!prev_segment->GetProjection(vec2d, &sl_point, &waypoint)) {   //计算车辆当前位置在历史平滑参考上的位置
  ...
  const double look_forward_required_distance =
      PncMap::LookForwardDistance(state.linear_velocity()); //前向查询距离
  if (remain_s > look_forward_required_distance) { //如果剩下的距离足够长，直接复用这条历史平滑参考线
  ...
  double future_end_s =
      prev_segment_length + FLAGS_look_forward_extend_distance; //向后额外扩展 FLAGS_look_forward_extend_distance的距离，默认为50m
  ...
  if (!pnc_map_->ExtendSegments(*prev_segment, future_start_s, future_end_s,
    ...
    return SmoothRouteSegment(*segments, reference_line);//扩展操作失败，直接对新的RouteSegment进行平滑得到平滑的参考线
  }
  lock.unlock();
  if (prev_segment->IsWaypointOnSegment(shifted_segments.LastWaypoint())) {
    *segments = *prev_segment;     //扩展操作成功，但是扩展以后长度没有太大变化，直接使用历史参考线
    segments->SetProperties(segment_properties);
    *reference_line = *prev_ref;
...
bool ReferenceLineProvider::Shrink(const common::SLPoint &sl,
  ...
  if (sl.s() > FLAGS_look_backward_distance * 1.5) {  //若车后参考线的距离大于后向查询距离的1.5倍，需要进行收缩
...
AnchorPoint ReferenceLineProvider::GetAnchorPoint(
  ...
  if (hdmap::RightBoundaryType(waypoint) == hdmap::LaneBoundaryType::CURB) {
    ...
    if (safe_lane_width < kEpislon) {
      ...
      // center_shift += 4* smoother_config_.curb_shift();
    }
    // 0.5 的含义？？？
  }
  if (hdmap::LeftBoundaryType(waypoint) == hdmap::LaneBoundaryType::CURB) {
    safe_lane_width -= smoother_config_.curb_shift();
    // curb_shift_ =0.2 default
    if (safe_lane_width < kEpislon) {
      ...
      // // center_shift -= 0.5 * smoother_config_.curb_shift();
      //  AINFO <<"center_shift "<<center_shift;
    }
  }
  center_shift -=0.5 * smoother_config_.curb_shift();
  ...
  //根据平移距离shifted_left_width 求出车辆在世界坐标系矫正后的位置
